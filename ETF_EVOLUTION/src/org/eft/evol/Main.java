...
//		experiment(loadedETFS, maxSize, navValues, null);
	...
	private static<T extends Unit> void experiment(List<ETF> loadedETFS, int maxSize, List<Map<Integer, Float>> navValues, Class<T> klass) {
		...
		while (it < iterationsMax) {
			...
			while (units.size() > 1 && cycle < maxSize) {
				...
				if (removed.get() > 0) {
					appendMessage("[" + cycle + "]Number of removed:" + removed + " from " + POPULATION_SIZE);
				}
				...
				Integer crossover = 0;
				if (toBeAdded > 0) {
					List<Unit> winners = getWinners(cycle, navValues, units, toBeAdded * 2);
					if (winners.size() == 1) {
						appendMessage("[" + cycle
						...
						while (units.size() != POPULATION_SIZE) {
							Unit newUnit = getUnitImplementation(winner);
							newUnit.mutate();
							mutated++;
					...
					appendMessage("[" + cycle + "]Number of normal units:"
							+ units.stream().filter(u -> u instanceof UnitImpl).count());
			...
			List<Unit> winners = getWinners(cycle - 1, navValues, units, 10);
	...
	private static Unit getUnitImplementation(Unit winner) {
		if (winner instanceof UnitChoosyImpl) {
			return new UnitChoosyImpl(UnitSequenceGenerator.getID(), winner.getCharacter(), winner.getPreferences());
		} else if (winner instanceof UnitSigmoidImpl) {
			return new UnitSigmoidImpl(UnitSequenceGenerator.getID(), winner.getCharacter(), winner.getPreferences());
		}
		return new UnitImpl(UnitSequenceGenerator.getID(), winner.getCharacter(), winner.getPreferences());
	}
	...
	private static void oneStep(List<Map<Integer, Float>> navValues, List<Unit> units, int it, int cycle, Unit unit,
		...
		if (cycle % 365 == 0) {
			float cashAdded = (float) (((int) (cycle / 120)) * (INVESTMENT / 2.0f));
