...
public class NetRelu {
	...
	public static void main(String[] args) throws IOException {
	    ...
	    DataSet trainingSet = TrainingSetCreatorRELU.getTrainingSet(etf, SIZE);
		...
		NeuralNetwork neuralNet = new MLPReluNet(SIZE,11,11,11,11,11,1);
	    	    
	    BackPropagation bpg = (BackPropagation) neuralNet.getLearningRule();
        bpg.setMaxError(0.00001d);
        ...
        for(DataSetRow row : subTrainingSet.getRows()){
        	...
        	double error = 0.5d*Math.pow(Math.abs(row.getDesiredOutput()[0] - out[0]),2);
        	if(error > maxError){
        		maxError = error;
        	}
        	sumError+=error;
        	System.out.println("calc"+Arrays.toString(row.getInput())+"->"+Arrays.toString(out)+Arrays.toString(row.getDesiredOutput())+"| error="+error);
        }
        
        System.out.println(String.format("Average error:%.7f | Max error:%.7f",sumError/((double) calculated.getRows().size()), maxError));
