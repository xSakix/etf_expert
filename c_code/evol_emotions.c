...
void decrease_etf_pref(unit u, int etf_index)
{
	float calc = (float)u->etfs_pref[etf_index] - 0.001f;
	if (calc > 0.0f) 
	{
...
void where_the_wind_blows(unit *population)
{
	float emotions[MAX_ETF_LENGTH];
	float max_units = (float)MAX_UNITS;
	for (int index_etf = 0; index_etf < MAX_ETF_LENGTH; index_etf++) {
		float sum = 0.0f;
		for (int index_pop = 0; index_pop < MAX_UNITS; index_pop++) {
			unit u = population[index_pop];
			float etf_preference = u->etfs_pref[index_etf];
			sum = (float)sum + etf_preference;
		}
		emotions[index_etf] = (float)sum / max_units;
	}

	for (int index_etf = 0; index_etf < MAX_ETF_LENGTH; index_etf++) {
		for (int index_pop = 0; index_pop < MAX_UNITS; index_pop++) {
			population[index_pop]->etfs_pref[index_etf] = emotions[index_etf];
		}
	}
...
int main()
{
	...
	for (int iter = 0; iter < MAX_ITER; iter++)
	{
		...
		for (int i = 0; i < MAX_DAYS; i++) {
			...
			where_the_wind_blows(population);
		}
