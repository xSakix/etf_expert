...
//6120 riadkov
typedef struct _etf_vals{
	unsigned int day;
	float nav[MAX_ETF_LENGTH];
}*etf_vals;

typedef struct _unit{		
	unsigned int ID;
	float etfs_pref[MAX_ETF_LENGTH];
	float character[CHARACTER_LENGTH];
	float cash;
	int portfolio[MAX_ETF_LENGTH];
} *unit;
...
void init_unit(unit u, unsigned int id){
	u->cash = INIT_CASH;	
	u->ID = id;
...
void do_buy_action(unit u, etf_vals value_field){
	...
	if(u->etfs_pref[etf_index] >= prob ){
		...
		float val = value_field->nav[etf_index];
		if((int)val == 0){
			return;
		}
		if(val > u->cash){
...
void do_sell_action(unit u, etf_vals value_field){
	int etf_index = random_at_most(MAX_ETF_LENGTH);
...
void oneStep(unit *population,etf_vals values ){
...
void load_etf_values(etf_vals *values,char **etf_names){
	DIR *dir;
	struct dirent *ent;
			...
			while ((read = getline(&line, &len, file)) != -1) {
					...
					while(token) {
						if(count == 0){
							...
							//printf("%d\n",index);
							values[index]->day=index;
							values[index]->nav[etf_index] = val;
						}
						token = strtok(NULL, " ");
					}
...
float compute_value(int day,unit u, etf_vals *values){
	float sum = u->cash;
	for(int i=0;i < MAX_ETF_LENGTH;i++){
		if(u->portfolio[i] == 0)
			continue;
		int days_iterator = day;
		float val = values[day]->nav[i];
		while((int)val == 0 && days_iterator > 0){
			val = values[days_iterator--]->nav[i];			
		}
		float ssum=(float)(val*u->portfolio[i]);
		printf("Summing:%f*%d=%f",val,u->portfolio[i],ssum);
		sum+=ssum;
	}
...
int main() 
{
	...
	etf_vals *values = malloc(MAX_DAYS*sizeof(etf_vals));
	...
	for(int i = 0;i < MAX_DAYS;i++){
		values[i] = malloc(sizeof(struct _etf_vals));
	}
	...
	int id_unit = 0;
	for(int i= 0;i < MAX_UNITS;i++){
		float sum = compute_value(MAX_DAYS-1,population[i],values);
		if(sum > maxSum){
			maxSum=sum;
			id_unit = i;
		}
